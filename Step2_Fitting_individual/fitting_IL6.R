...
parser<-OptionParser()
parser<-add_option(parser, c("-a", "--patient"), default="M68",type="character", help="patient ID")
...
parser<-add_option(parser, c("-l", "--lambda"), default=10,type="integer", help="Population size to use for CMA-ES [default 4+floor(3*log(N))]")
parser<-add_option(parser, c("-m", "--maxiter"), default=10,type="integer", help="Maximum number of generations for CMA-ES [default 100*N^2]")
...
tmp <- read.delim(paste0(outdir1,"/results_CAR/","pars.txt"),header = FALSE, sep = ' ')
CAR_Parm = as.numeric(tmp$V2)
names(CAR_Parm) = tmp$V1	
#initial guesses
pp<-read.delim("mmm.txt",header = TRUE, sep = '\t') #../nnn.txt is the file used by Step0; nnn.txt is a file
...
for (i in 1:nrow(pp)){
	if (pp$Parameter[i] %in% Pre_fitted$X){
		...
		pp$Low[i] <- as.numeric(Pre_fitted[which(Pre_fitted$X==pp$Parameter[i]),patientID])/ 100
		pp$High[i] <- as.numeric(Pre_fitted[which(Pre_fitted$X==pp$Parameter[i]),patientID]) * 100
	}
}
...
objfun<-function(ind){
	...
	parsin <- c(parsin, CAR_Parm)  # add the CAR parameters estimated previously	
	#names(parsin)=names(ind)
	parnames = names(parsin)
	replaced_pars <- c(parnames) # makes weight get updated per individual patient's weight
...
estimatedpars_CAR<-read.table(paste0(outdir1,"/results_CAR/pars",".txt"),header=F,as.is=T)
estimatedpars_IL6<-read.table(paste0(outdir1,"/results_IL6/pars",".txt"),header=F,as.is=T)

objfun<-function(parsin){
	#no need to do decode
	parsvec<-parsin[,2]
	names(parsvec)=parsin[,1]
	parsin <- c(parsin, CAR_Parm)
...
forplotting<-objfun(rbind(estimatedpars_CAR,estimatedpars_IL6))
source('Plotting.R')